...
- ⚙️ **Settings → General: ส่วน Watchdog** — เลือกได้ว่าเมื่อกด Exit
  จะเขียน marker กัน watchdog หรือไม่ (ค่าเริ่มต้น: เขียน = แอปอยู่ปิด)
  ปิดได้สำหรับคนที่อยากให้ watchdog ปลุกแอปกลับเสมอ พร้อมสถานะ task
  แบบสด 3 สถานะ (เปิดอยู่ / ปิดอยู่ / ไม่ได้ติดตั้ง) ครบ 4 ภาษา
...
- 🧩 **Lite: หน้าต่าง Details ปิดไม่ได้** — `MessageBoxW` ถูกเรียกในเธรด
  message loop ของ pystray → modal box บล็อก loop ทำให้ tray ค้างและกล่อง
  ปิดไม่ลง (บั๊กเดียวกับแอปหลัก v1.14 ที่เคยแก้) — ตอนนี้กล่องเปิดบนเธรด
  ของตัวเอง (เมนูคืนทันที) + ธงกันเปิดซ้อน และ Refresh ในเมนูก็ย้ายออกจาก
  เธรดเมนูด้วย (SMART ใช้เวลาถึง 30 วิ) — พิสูจน์: เปิดกล่อง → ปิดผ่าน
  WM_CLOSE สำเร็จขณะ main thread ยังตอบสนอง
