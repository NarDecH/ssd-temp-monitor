...
แท็บ Health มีปุ่ม **รายงานรายสัปดาห์** (เปิดในเบราว์เซอร์ทันที),
**ส่งออก...** (เลือกที่เซฟเองด้วย Save As) และ **ส่งออก CSV** (ไฟล์ที่
Excel เปิดแล้วคอลัมน์ตรงทันที — UTF-8 BOM + `sep=,`) — ไฟล์ HTML ไฟล์เดียวจบ
ไม่พึ่งเน็ต ส่งต่อ/แนบเมลได้ พร้อม **กราฟ 3 ชุดต่อดิสก์**: อุณหภูมิเฉลี่ยรายวัน /
การสึก (wear %) / uncorrected read errors (ตัดที่ 500) และเครื่องที่มี SSD
ตั้งแต่ 2 ตัวขึ้นไปจะมี**กราฟเปรียบเทียบทุกดิสก์ในภาพเดียว** (สีต่างกัน + legend)
ข้อมูลมาจาก `health_daily.csv` ที่แอปบันทึกวันละหนึ่งแถวต่อดิสก์อัตโนมัติ

นอกจากนี้เปิด**รายงานอัตโนมัติทุก 7 วัน**ได้ใน Settings → General — แอปจะสร้าง
รายงานลงโฟลเดอร์ที่กำหนดพร้อมแจ้ง toast เมื่อบันทึก

## กราฟ 24 ชั่วโมง และโหมดมืด/สว่าง

เมนู tray → **ประวัติย้อน 24 ชั่วโมง**: ดูอุณหภูมิย้อน 24 ชม. เต็มจากข้อมูล
รายนาที (เก็บใน DATA_DIR — portable ยกไปได้) พร้อมสรุป min/avg/max
รีเฟรชเองทุก 30 วินาที และส่งออกเป็นรูป PNG ได้จากปุ่มในหน้าต่าง

พื้นหลังกราฟทุกหน้าต่างแบ่งเป็น**โซนสี**เขียว/ส้ม/แดงจาง ๆ ตามเกณฑ์เดียวกับ
สี tray (<51 / 51–64 / ≥65 °C) เห็นระดับอุณหภูมิได้ทันที

Settings → General → **ธีมหน้าต่าง** เลือก `auto` (ตามธีมแอปของ Windows)
/ `dark` / `light` — ใช้กับหน้าต่างกราฟ หน้า Settings และแท็บ Stats

## แท็บสถิติ

Settings → **สถิติ**: สรุป 7 วัน — จำนวนครั้งที่แจ้งเตือนอุณหภูมิเกินและ
SMART (อ่านจาก event log จริง) + ต่อดิสก์: ต่ำสุด/เฉลี่ย/สูงสุด/การสึก
พร้อมปุ่มรีเฟรช หรือเปิดเร็ว ๆ จากเมนู tray → **สถิติการใช้งาน**
...
| [faq.html](faq.html) | คำถามที่พบบ่อย (ไอคอน --, USB reader, auto-update, portable) |
